...
ettm1.ts = ts(data=ettm1$OT, start=c(2016, first_hour_etth1), freq=4*24*365)
...
```{r}
par(mfrow=c(1,2))
etth1_time = time(etth1.ts)
reg1 = lm(etth1.ts~etth1_time+I(etth1_time^5)+I(etth1_time^6))

plot(etth1.ts)
points(etth1_time, predict.lm(reg1),type='l',col='blue')

# dfetth1 = data.frame(ot = etth1$OT, week = as.integer(strftime(etth1$date, format = "%V")), year = as.integer(strftime(etth1$date, format = "%G")))
# unique(dfetth1$year)
# gam_model1 = gam(ot ~ s(year)+s(week), data = dfetth1)
# 
# plot(gam_model1)
...
f2 = 2
d = 24*365
v1 = cos(2*base::pi*f1*1:length(etth1.ts)/d)
v2 = sin(2*base::pi*f1*1:length(etth1.ts)/d)
v3 = cos(2*base::pi*f2*1:length(etth1.ts)/d)
v4 = sin(2*base::pi*f2*1:length(etth1.ts)/d)

seasonal_fit1 = lm(seasonality_etth1 ~ 1 + v1 + v2 + v3 + v4)
plot(1:length(etth1.ts), seasonality_etth1, type="l")
points(1:length(etth1.ts), seasonal_fit1$fitted, type = "l", col = "firebrick",lwd=3)

after_trend_seasonality_etth1 = seasonality_etth1-seasonal_fit1$fitted


gam1 = gam(after_trend_seasonality_etth1 ~ s(time(etth1.ts)))

plot(1:length(etth1.ts), after_trend_seasonality_etth1, type="l")
points(1:length(etth1.ts), gam1$fitted.values, type = "l", col = "firebrick",lwd=3)
...
# suggests differencing once is sufficient
acf(diff(diff(after_trend_seasonality_etth1-gam1$fitted.values,lag=1),lag=24*365), lag.max=200)

# ettm1

diff_ettm1.ts = diff(ettm1.ts)
diff2_ettm1.ts = diff(diff(ettm1.ts))
diff3_ettm1.ts = diff(diff(diff(ettm1.ts)))
acf(diff_ettm1.ts, lag.max=100)
acf(diff2_ettm1.ts,lag.max=100)
...
etth1_model3 = arima(etth1.ts, order = c(0,1,2), seasonal=list(order=c(0,1,1), period=24))
plot(etth1.ts)
