...
# significant lag 1 and 2 components. Trying MA component
# seasonal ma are along the edge of unit circle
etth2_model3 = arima(etth2.ts, order = c(1,3,1), seasonal=list(order=c(0,1,1), period=24))
plot(etth2.ts)
...
get_evaluation_on_ts = function(input_ts=etth1.ts, original_input_ts=etth1.ts.original,include_trend_seasonality=F) {
  ...
  evaluated_model1 = arima(train, order = c(1,3,2), seasonal=list(order=c(0,1,2), period=24))
  ...
  if (include_trend_seasonality) {
    ...
    evaluated_model1 = arima(train, order = c(0,3,1), seasonal=list(order=c(0,1,1), period=24))
...
get_evaluation_on_ts(etth1.ts,include_trend_seasonality=T)
